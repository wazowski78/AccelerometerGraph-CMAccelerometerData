...
@synthesize x, y, z, adaptive,acel2;
...
-(void)addAccelerationFilterCM:(CMAccelerometerData*)accel
{
	...
	if(adaptive)
	{
		double d = Clamp(fabs(Norm(acel2.x, acel2.y, acel2.z) - Norm(accel.acceleration.x, accel.acceleration.y, accel.acceleration.z)) / kAccelerometerMinStep - 1.0, 0.0, 1.0);
		alpha = (1.0 - d) * filterConstant / kAccelerometerNoiseAttenuation + d * filterConstant;
	}
	
	acel2.x = accel.acceleration.x * alpha + acel2.x * (1.0 - alpha);
	acel2.y = accel.acceleration.y * alpha + acel2.y * (1.0 - alpha);
	acel2.z = accel.acceleration.z * alpha + acel2.z * (1.0 - alpha);
}
...
-(void)addAccelerationFilterCM:(CMAccelerometerData*)accel
{
	...
	acel2.x = alpha * (acel2.x + accel.acceleration.x - lastX);
	acel2.y = alpha * (acel2.y + accel.acceleration.y - lastY);
	acel2.z = alpha * (acel2.z + accel.acceleration.z - lastZ);
